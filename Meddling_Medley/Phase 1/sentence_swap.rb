...
#  where every word is replaced with it's corresponding value in the 
#  hash. If a word does not exist as a key of the hash, then it should 
#  remain unchanged.

# Examples

def sentence_swap(sent,hash)
    return sent.split(" ").map{|word| hash.has_key?(word) ? hash[word] : word}.join(" ")
end
